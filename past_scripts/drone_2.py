...
"""
A drone
- Determine any Document that goes into the same Record, Given a root path 
- computeRecordIdentifier
    -> return an ID obj that contains
        - date(timestamp)
        - hash of the filename
        - hash unique to the state of the file
    -> documents 
- A method that determines which Record gets updated
- CreateDoc
- Update to database
